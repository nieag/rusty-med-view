...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if uniforms.view_mode > 0u {
        ...
        // Apply zoom and pan: scale UV around the stored center (zoom_pivot)
        ...
        if dist_z > 0.0 {
            ...
            // Map back to 0..1 virtual UV space
            let p_uv = vec2<f32>(screen_u + 0.5, screen_v + 0.5);
            
            // Re-apply Zoom/Pan to find the actual pixel on screen
